 ...
 * few bytes of parsing at the beginning of the connection, to look for the SNI, the whole
 ...
 * </code> and a <code>write</code> operation can run out of both bytes for reading and
...
public interface TlsChannel extends ByteChannel, GatheringByteChannel, ScatteringByteChannel {
     ...
     *
     * <p>An attempt is made to read up to <i>r</i> bytes from the channel, where <i>r</i> is
     * the number of bytes remaining in the buffer, that is, <code>dst.remaining()</code>, at
     ...
     *
     * <p>A read operation might not fill the buffer, and in fact it might not read any bytes
     ...
     * NeedsWriteException}, due to the fact that, during a TLS handshake, bytes need to be
     ...
     * @return The number of bytes read, or <code>-1</code> if the channel has reached
     ...
     * @throws SSLException if the {@link SSLEngine} throws an SSLException
     ...
     *
     * <p>An attempt is made to write up to <i>r</i> bytes to the channel, where <i>r</i> is
     * the number of bytes remaining in the buffer, that is, <code>src.remaining()</code>, at
     * the moment this method is invoked.
     *
     ...
     * writing all the <i>r</i> requested bytes. On the other hand, if it is in non-blocking
     * mode, this operation may write only some of the bytes or possibly none at all, in which
     ...
     * @return The number of bytes written, contrary to the behavior specified in {@link
     ...
     *
     * <p>This method may block if the underlying channel is in blocking mode.
     *
     * <p>Note that renegotiation is a problematic feature of the TLS protocol, and should only be
     * initiated at a quiet point of the protocol.
     *
     ...
     * @param offset The offset within the buffer array of the first buffer from which bytes
     ...
     * @param offset The offset within the buffer array of the first buffer into which bytes
     ...
     * to prevent truncation attacks, which consist, essentially, of an adversary introducing TCP FIN
     * segments to trick one party into ignoring the final bytes of a secure stream. For more details, see
